...
describe("PREDICTIONS P2P contract tests", function () {
    ...
    beforeEach(async ()=>{
        ...
        const [FactoryEquivalence,FactoryRouter, FactoryToken,FactoryBold,FactoryPredictions,FactoryMockAgregatorV3] = await Promise.all([
           ...
           ethers.getContractFactory("OFPrediction"),
  ...
  it("Test EXECUTEROUND after bufferSeconds OPERATOR", async ()=> {
   ...
   // *******************************************************************
   //    REFUNDABLE, CLAIMABLE, BONUSCLAIMABLE
   // *******************************************************************
   ...
   it("Test CLAIMABLE ROUND", async ()=> {
      ...
      claimable=await predictionContract.claimable(1,UserBet1Address);
      ...
      claimable=await predictionContract.claimable(1,UserBet2Address);
      expect(claimable).to.be.false;
      claimable=await predictionContract.claimable(2,UserBet1Address);
      expect(claimable).to.be.false;
      claimable=await predictionContract.claimable(2,UserBet2Address);
      expect(claimable).to.be.false;
      ...
      claimable=await predictionContract.claimable(2,UserBet1Address);
      expect(claimable).to.be.true; // Even round is BULL as is the only better, round is claimable
      claimable=await predictionContract.claimable(2,UserBet2Address);
      expect(claimable).to.be.false;
      claimable=await predictionContract.claimable(3,UserBet1Address);
      expect(claimable).to.be.false;
      claimable=await predictionContract.claimable(3,UserBet2Address);
      expect(claimable).to.be.false;
   });
   ...
   it("Test BONUS CLAIMABLE ROUND", async ()=> {
      let roundId=1,answer=1000000,updatedAt=2;
      let betAmount=minBetAmount;
      let bonusClaimable;
      let blockInfo=await mockAgregator.getBlockInfo();
      ...
      await predictionContract.connect(UserBet1).betBull(1,betAmount)
      await predictionContract.connect(UserBet2).betBear(1,betAmount)
      bonusClaimable=await predictionContract.bonusClaimable(1,UserBet1Address);
      expect(bonusClaimable).to.be.false;
      bonusClaimable=await predictionContract.bonusClaimable(1,UserBet2Address);
      expect(bonusClaimable).to.be.false;
      ...
      bonusClaimable=await predictionContract.bonusClaimable(1,UserBet2Address);
      expect(bonusClaimable).to.be.false;
      bonusClaimable=await predictionContract.bonusClaimable(2,UserBet1Address);
      expect(bonusClaimable).to.be.false;
      bonusClaimable=await predictionContract.bonusClaimable(2,UserBet2Address);
      expect(bonusClaimable).to.be.false;
      ...
      bonusClaimable=await predictionContract.bonusClaimable(1,UserBet1Address);
      expect(bonusClaimable).to.be.true;
      bonusClaimable=await predictionContract.bonusClaimable(1,UserBet2Address);
      expect(bonusClaimable).to.be.true;
      bonusClaimable=await predictionContract.bonusClaimable(2,UserBet1Address);
      expect(bonusClaimable).to.be.true; 
      ...
      // As NOT block.timestamp > round.closeTimestamp + bufferSeconds even round is marked as refundable,
      // are not refundable
      bonusClaimable=await predictionContract.bonusClaimable(3,UserBet1Address);
      expect(bonusClaimable).to.be.false;
      bonusClaimable=await predictionContract.bonusClaimable(3,UserBet2Address);
      expect(bonusClaimable).to.be.false;
   });
   ...
   it("Test CLAIM ROUND", async ()=> {
      ...
      expect(routerBalance).to.eq(betAmount); //User 1 has bet
      userBalance=await tokenContract.balanceOf(UserBet2.address);
      await predictionContract.connect(UserBet2).betBear(1,betAmount)
      let newUserBalance=await tokenContract.balanceOf(UserBet2Address);
      expect(newUserBalance.toString()).to.eq(userBalance.sub(betAmount).toString());
      let newRouterBalance=await tokenContract.balanceOf(routerContract.address);
      expect(newRouterBalance).to.eq(betAmount*2);
      ...
      .withArgs(UserBet2Address,1,betAmountExFees*2);
      claimable=await predictionContract.claimable(1,UserBet2Address);
      expect(claimable).to.be.false;
      newUserBalance=await tokenContract.balanceOf(UserBet2Address);
      expect(newUserBalance.toString()).to.eq(userBalance.add(betAmountExFees*2).toString());
      newRouterBalance=await tokenContract.balanceOf(routerContract.address);
      expect(newRouterBalance).to.eq(routerBalance.sub(betAmountExFees*2));
      ...
      userBalance=await tokenContract.balanceOf(UserBet1Address);
      await expect(predictionContract.connect(UserBet1).claim([2,3]))
      .to.emit(predictionContract, "Claim");
      newuserBalance=await tokenContract.balanceOf(UserBet1Address);
      expect(newuserBalance).to.eq(userBalance.add(betAmountExFees*4));
 
      await expect(predictionContract.connect(Admin).claimTreasury())
      .to.emit(predictionContract, "TreasuryClaim")
      .withArgs(6*betFeeAmount);
      newRouterBalance=await tokenContract.balanceOf(routerContract.address);
   ...
   it("Test CLAIM BONUS rewards", async ()=> {
      ...
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([0]))
      .to.be.revertedWith('Round has not started');
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([1]))
      .to.be.revertedWith('Round has not ended');
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([2]))
      .to.be.revertedWith('Round has not started');
      ...
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([2]))
      .to.be.revertedWith('Round has not ended');
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([3]))
      .to.be.revertedWith('Round has not started');
      ...
      .to.be.revertedWith('Round has not ended');
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([1]))
      .to.emit(predictionContract, "ClaimBonus")
      .withArgs(UserBet1Address,1,betAmount);
      await expect(predictionContract.connect(UserBet1).claimBonusRewards([1]))
      .to.be.revertedWith('Not eligible for bonus claim');

      // User 2 can claim
      userBalance=await boldContract.balanceOf(UserBet2Address);
      await expect(predictionContract.connect(UserBet2).claimBonusRewards([1]))
      .to.emit(predictionContract, "ClaimBonus")
      .withArgs(UserBet2Address,1,betAmount);
      newUserBalance=await boldContract.balanceOf(UserBet2Address);
      expect(newUserBalance.toString()).to.eq("310387425584000");
      ...
      await predictionContract.connect(UserBet1).claimBonusRewards([2,3]);
      newUserBalance=await boldContract.balanceOf(UserBet1Address);
      expect(newUserBalance.div(1000)).to.eq("931162276753");
      newRouterBalance=await boldContract.balanceOf(routerContract.address);
      expect(newRouterBalance).to.eq(currentRouterBalance);

      await expect(predictionContract.connect(UserBet1).claimBonusRewards([2,3]))
      .to.be.revertedWith('Not eligible for bonus claim');
   ...
   it("Test REFUND (BETS IN ONE SIDE)", async ()=> {
      ...
      await expect(predictionContract.connect(UserBet1).claim([2]))
      .to.emit(predictionContract, "Claim")
      .withArgs(UserBet1Address,2,betAmountExFees*2);
   });
